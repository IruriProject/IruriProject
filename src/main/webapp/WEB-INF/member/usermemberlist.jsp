...
<h4>일반회원명단(총<b style="color: #4E9F3D; text-align:center;">&nbsp;${totalCount }</b> 명)</h4>
<table class="table table-info" style="width: 100%;">
		<tr>
			<th style="text-align:center; width: 50px;">번호</th>
			<th style="text-align:center;width: 90px;">이름</th>
			<th style="text-align:center; width: 130px;">아이디</th>
			<th style="text-align:center; width: 130px;">핸드폰</th>
			<th style="text-align:center; width: 350px;">주소</th>
			<th style="text-align:center; width: 200px;">이메일</th>
			<th style="text-align:center; width: 30px;">
			<input type="checkbox" id="allcheck" class="chkbox"></th>
		</tr>
		
		<c:forEach var="dto" items="${list }" varStatus="i">
			<tr >
				<td style="text-align:center;">${i.count }</td>
				<td style="text-align:center;">${dto.u_name}</td>
				<td>${dto.u_id }</td>
				<td style="text-align:center;">${dto.u_hp }</td>
				<td>${dto.u_addr }</td>
				<td>${dto.u_email }</td>
				
				<td><input type="checkbox" class="del chkbox" name="chk"  num="${dto.u_num }"></td>
		...
		<c:if test="${totalCount>0}">
		 <div class="pagination" style=" display: flex; justify-content: center; width:100%; text-align: center;">
					...
					<c:if test="${startPage>1 }">
						<a href="usermemberlist?currentPage=${startPage-1}">&laquo;</a>
					</c:if>

					<c:forEach var="pp" begin="${startPage }" end="${endPage }">
						<c:if test="${currentPage==pp }">
							<a class="active" href="usermemberlist?currentPage=${pp}">${pp}</a>							
						</c:if>

						<c:if test="${currentPage!=pp }">
							<a href="usermemberlist?currentPage=${pp}">${pp}</a>
						</c:if>
					</c:forEach>
					
					<!--다음 -->
					<c:if test="${endPage<totalPage }">
						<a href="usermemberlist?currentPage=${endPage+1}">&raquo;</a>					
					</c:if>
			</div>
		</c:if>
...
<script type="text/javascript">
    function deleteMembers() {
        ...
        $.ajax({
            ...
            dataType: "html",
            url: "/member/delete", // 삭제 요청을 처리하는 URL을 지정합니다.
            data: {"u_num": checkedMembers.join(",")},
            success: function () {
            	alert("삭제되었습니다.");
                location.reload();
            }
