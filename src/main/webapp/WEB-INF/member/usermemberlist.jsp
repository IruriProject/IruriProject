			...
			<div style="width: 800px; text-align: center;">
				<ul class="pagination">
					...
					<c:if test="${startPage>1 }">
						<li>
						<a href="usermemberlist?currentPage=${startPage-1}">이전</a>
						</li>
					</c:if>

					<c:forEach var="pp" begin="${startPage }" end="${endPage }">
						<c:if test="${currentPage==pp }">
							<li class="active"><a href="usermemberlist?currentPage=${pp}">${pp}</a>
							</li>
						</c:if>

						<c:if test="${currentPage!=pp }">
							<li><a href="usermemberlist?currentPage=${pp}">${pp}</a></li>
						</c:if>
					</c:forEach>
					...
					<c:if test="${endPage<totalPage }">
						<li>
						<a href="usermemberlist?currentPage=${endPage+1}">다음</a>
						</li>
					</c:if>
				</ul>
			</div>
