	...
	<table class="table" style="width: 100%; margin: 0 auto;">
		...
		<div class="pagination" style=" display: flex; justify-content: center; width:100%; text-align: center;">
			...
			<c:if test="${startPage > 1 }">
				<a href="/enterprise/viewerlist?p_num=${p_num }&currentPage=${startPage - 1 }">&laquo;</a>
			</c:if>
			
			<c:forEach var="pp" begin="${startPage }" end="${endPage }">
				<c:if test="${pp == currentPage }">
					<a class="active" href="/enterprise/viewerlist?p_num=${p_num }&currentPage=${pp }">${pp }</a>
				</c:if>
				<c:if test="${pp != currentPage }">
					<a href="/enterprise/viewerlist?p_num=${p_num }&currentPage=${pp }">${pp }</a>
				</c:if>
			</c:forEach>

			<!-- 다음 -->
			<c:if test="${endPage < totalPage }">
				<a href="/enterprise/viewerlist?p_num=${p_num }&currentPage=${endPage + 1 }">&raquo;</a>
			</c:if>
